...
print_expr(int pr, int opn)
{
   ...
   if (opn < RX) {                      // Immediate value.
      if (-31 <= r[opn] && r[opn] <= 31) {
         printb(pr, "%d", r[opn]);
      } else {
         printb(pr, "0x%X", r[opn]);
      }
   }
   else if (opn == RX) {
      printb(pr, "x");     // First argument.
   }
#if NARGS >= 2
   else if (opn == RY) {
      printb(pr, "y");     // Second argument.
   }
#endif
      ...
      for (j = 0; j < isa[k].numopnds; j++) {
         ...
         if (j < isa[k].numopnds - 1) {
            printb(pr, "%s", isa[k].op_name);
         } else {
            printb(pr, ")");
         }
      }
...
#endif
   else if (opnd == numi + RI0 - 1) {
      printb(pr, "%%1");
   } else {
      printb(pr, "%s", reg_names[opnd - RI0]);
   }
}
...
print_pgm(int pr)
{
   ...
   for (i = 0; i < numi; i++) {
      ...
      // x86 instructions will have one less operand than those in the
      // ISA this code was initially designed for, since x86 uses the
      // first source operand as the destination. So, we check to make
      // sure that the number of operands is nonzero, so we can perform
      // a proper translation.
      assert(isa[current_op].numopnds > 0);

      printb(pr, "   \"%-5s ", "mov", i + 1);
      print_opnd(pr, pgm[i].opnd[0]);
      printb(pr, ", ");
      print_opnd(pr, i + RI0);
      printb(pr, ";\"\n");
      printb(pr, "   \"%-5s ", isa[current_op].mnemonic);

      for (j = 1; j < isa[current_op].numopnds; j++) {
         opndj = pgm[i].opnd[j];
